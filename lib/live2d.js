...
const L2D = {
  ...
  async init() {
    ...
    await this._waitForCubismCoreReady();
  ...
  // Cubism 4（WASM 版 Core，live2dcubismcore.min.js）的 <script> 標籤執行完，
  // Live2DCubismCore.Version/.Moc/.Model... 這些 API 就同步掛好了，但底層真正的
  // WASM runtime（WebAssembly.instantiate + Emscripten 的 onRuntimeInitialized）
  // 是非同步才完成初始化的——_checkDeps() 只檢查 `typeof Live2DCubismCore !==
  // 'undefined'`，太早就判定「可以用了」。如果 _load() 搶在 WASM runtime 真的
  // 就緒前呼叫到 Cubism Core（例如建立 Moc/Model），會撞上 Emscripten 那句經典的
  // 「native function called before runtime initialization」，但這裡是被
  // _load() 外層 try/catch 吞掉、還是繼續往下跑，觀察到的症狀就是「角色第一次
  // 載入時某些部位（例如臉部/頭部）貼圖/材質缺漏」，重新整理（F8）就正常——第二次
  // 之所以會好，純粹是腳本已經被瀏覽器編譯快取過一次，這次 WASM 初始化跑得夠快，
  // 湊巧在 _load() 真的呼叫到 Cubism Core 之前就完成，不是巧合修好、是時間差恰好
  // 沒撞上。
  //
  // 用「真的呼叫一次最輕量的 Cubism Core API（取版本號），失敗/拿到假值就短暫等待
  // 重試」來確認 runtime 真的就緒，比單純檢查全域變數存不存在可靠，也不用去動
  // live2dcubismcore.min.js／lib/all.min.js 這兩個第三方檔案本身——跟
  // _patchClippingMaskOverflow() 同一種「在 lib/live2d.js 加執行期防呆，不改
  // 第三方檔案」的既有作法（見該函式與 furina-顯示異常除錯筆記.md 的說明）。
  // 有次數上限（40 次 × 50ms＝2 秒）避免這個判斷方式本身失準時卡死整個桌寵啟動，
  // 超過上限就印警告照樣往下跑，不讓角色永遠載入不出來。Cubism 2（Live2D 全域，
  // 非 WASM）沒有這個非同步初始化問題，不用等。
  async _waitForCubismCoreReady() {
    if (typeof Live2DCubismCore === 'undefined') return;
    for (let i = 0; i < 40; i++) {
      try {
        if (Live2DCubismCore.Version.csmGetVersion()) return;
      } catch {}
      await new Promise((resolve) => setTimeout(resolve, 50));
    }
    console.warn('[L2D] 等待 Cubism Core（WASM）就緒逾時，繼續載入角色（可能撞上首次載入偶發缺漏的問題）');
  },
