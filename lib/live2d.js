...
// ══════════════════════════════════════════════════════════════════════════════
const L2D_CFG = {
  // manifest.json 路徑 + 消費端可載入的角色數上限（見 L2D.init() 的 slots 裁切邏輯）
  ...
  // null = 兩角色都動 | 'c1' 只讓角色一動 | 'c2' 只讓角色二動
...
const L2D = {
  app: null,
  _c1: { model: null, gMap: {}, expList: [], expQueue: [], lastExpIdx: -1, ready: false },
  _c2: { model: null, gMap: {}, expList: [], expQueue: [], lastExpIdx: -1, ready: false },
  _c1LastName: null,   _c1SeqIdx: -1,
  ...
  async init() {
    ...
    const manifest = await fetch(L2D_CFG.manifestUrl).then(r => r.json()).catch(err => {
    ...
    const allSlots = [
      { key: 'c1', cfg: L2D_CFG.char1, side: 'left' },
      { key: 'c2', cfg: L2D_CFG.char2, side: 'right' },
    ];
    ...
    for (const { key, cfg } of slots) {
      if (cfg.model) continue;   // 已有 override，manifest 解析結果不覆蓋
