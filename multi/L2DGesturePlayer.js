 ...
 *
 * 公開 API:
 * L2DGesturePlayer.play(name)          ← 播放已命名的動作
...
const L2DGesturePlayer = (() => {
  ...
  const GESTURES = {
    ...
    'surprise': {
      char: 'c2', blend: 0.8, // 高 Blend 讓動作幾乎瞬間到位
      frames: [
    ...
    'slump': {
      char: 'c2', blend: 0.15, // 低 Blend 營造無力沉重感
      frames: [
  ...
  function _hookChar(charKey, gesture) {
    ...
    if (gesture.type === 'fn') {
      const totalDur  = gesture.duration ?? 1500;
    ...
    // ── 原有 keyframe 插值模式 ────────────────────────────────────────────────
    const totalDur  = gesture.duration ?? gesture.frames[gesture.frames.length - 1].t;
  ...
  return {
    play(name) {
      ...
      if (!g) { console.warn('[GesturePlayer] 未知動作:', name, '| 可用:', Object.keys(GESTURES).join(', ')); return; }
      const chars = g.char === 'both' ? ['c1', 'c2'] : [g.char ?? 'c2'];
